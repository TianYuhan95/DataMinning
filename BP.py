...
for i in range(len(layer)-1):
    ...
    b.append(np.array([0.1]*layer[i+1])) #偏置元b[i]的规模是1*第1+i个隐藏层节点数
a = [np.array(0)]*(len(W)+1) #a[0] = x,即输入,a[1]=f(z[0]),a[len(W)+1] = 最终输出
z = [np.array(0)]*len(W) #注意z[0]表示网络输入层的输出,即未被激活的第一个隐藏层
W = np.array(W)
...
diff_activation_function = diff_sigmoid #设置激活函数导数

#开始训练BP神经
a[0] = np.array(trainingSet)
...
for v in range(num_passes):
    ...
    predict = a[-1][0] #a[-1]是输出层结果,即为预测值
    ...
    for i in range(len(W)):
        #使用矩阵运算简化公式,下面两行代码已将全部样本反向传播得到的偏导数值求和
    ...
    print '训练样本的错误率:',error_rate(predict,np.array(trainingLabels))
    ...
    print '测试样本的错误率:',error_rate(predict,np.array(testLabels))
